...
describe("Resource catalog tools", () => {
  it("registers 133 verified fixed data.gov.in tools with unique names", () => {
    expect(RESOURCE_TOOL_DEFS).toHaveLength(133);
    expect(RESOURCE_TOOL_NAMES).toHaveLength(133);
    expect(new Set(RESOURCE_TOOL_NAMES).size).toBe(133);
  });
  ...
  it("exposes catalog tools plus elevation and postal_code on the server tool list", () => {
    ...
    expect(toolNames.length).toBe(25 + 2 + 133 + 28);
  });
});
